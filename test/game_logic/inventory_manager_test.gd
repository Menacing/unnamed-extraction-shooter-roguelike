...
func test__on_game_saving() -> void:
	...
	assert_int(player.player_inventory_id).is_equal(test_inv.inventory_id)
	...
	#save data test
	var save_file:SaveFile = SaveManager.save_game("test_values_save_and_load_correctly")
	
	assert_int(save_file.inventories.front().grid_slots[0][0]).is_equal(start_item_instance_id)
