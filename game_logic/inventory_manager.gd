	...
	if item_type_id and inventory:
		...
		for w in inventory.grid_slots:
			for h in inventory.grid_slots[w]:
				var item_inst_id = inventory.grid_slots[w][h]
				if item_inst_id:
					var item_instance:ItemInstance = ItemAccess.get_item_instance(item_inst_id)
					if item_instance.get_item_type_id() == item_type_id and !checked_item_instance_ids.has(item_instance.item_instance_id):
						count += 1
						checked_item_instance_ids.append(item_instance)
